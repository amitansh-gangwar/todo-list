...
import (
	...
	"github.com/gangwar-2/todo-list/db"
...
func TestCreate(t *testing.T) {
	...
	task_id, err := Create(dbIns, "testing task")
	assert.NoError(t, err)

	s, err := dbIns.Prepare("SELECT task FROM tasks WHERE task_id=$1")
	assert.NoError(t, err)
	...
	assert.Equal(t, "testing task", task)
...
func TestCannotCreateEmptyTask(t *testing.T) {
	...
	assert.EqualError(t, err, "Cannot create an empty tasks")
